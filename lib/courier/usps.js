...
var parser = {
  trace: function (body) {
    ...
    $history.each((index, element) => {
      ...
      if (checkpoint.message.includes('Pre-Shipment Info Sent to USPS')) checkpoint.status = tracker.STATUS.PENDING;
      if (checkpoint.message.includes('Shipping Label Created')) checkpoint.status = tracker.STATUS.INFO_RECEIVED;
      if (checkpoint.message.includes('Delivered')) checkpoint.status = tracker.STATUS.DELIVERED;
      if (checkpoint.message.includes('Label Cancelled')) checkpoint.status = tracker.STATUS.CANCELED;
